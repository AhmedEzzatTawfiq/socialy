    ...
    <div className='min-h-screen bg-linear-to-b from-slate-50-50 to-white'>
      <div className='max-w-6xl mx-auto p-6'>
        ...
        <div className='max-w-xl bg-white p-4 sm:p-8 rounded-xl shadow-md space-y-4'>
          ...
          <textarea name="" id="" className='w-full resize-none max-h-20 mt-4 text-sm outline-none placeholder-gray-400'
            placeholder='What`s happening'
            onChange={(e) => setContent((e.target.value))} value={content}></textarea>
          {
            images.length > 0 && <div className='flex flex-wrap gap-2 mt-2'>
              {
                images.map((image, i) => (
                  <div key={i} className='relative group'>
                    <img src={URL.createObjectURL(image)} alt="" className='h-20 rounded-md' />
                    <div onClick={() => setImages(image.filter((_, index) => index !== i))}
          ...
          <div className='flex items-center justify-between pt-3 border-t text-gray-300'>
            ...
            <input type="file" id='images' name="images" accept='images/*' hidden multiple
              onChange={(e) => setImages([...images, ...e.target.files])} />         {/*important*/}
            <button onClick={() => toast.promise(
              handleSubmit(), {
              loading: "Uploading...",
              error: <p>Post Not Added</p>
            }
            )} disabled={loading} className='text-sm bg-linear-to-r from-indigo-500 to-purple-600 hover:from-indigo-600 hover:to-purple-700 
